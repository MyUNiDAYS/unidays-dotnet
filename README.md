...
|---|---|---|---|
| PartnerId | Your PartnerId as provided by UNiDAYS. If you operate in multiple geographic regions you MAY have a different PartnerId for each region | Base64 Encoded String | XaxptFh0sK8Co6pI== |
...
It is a mandatory requirement that all server URLs are signed. This means you are required to pass the signing key UNiDAYS provide you with as one of the arguments. The signing key is a Base64 string. This endpoint accepts both `GET` and `POST` requests.
...
namespace test
{
    class Program
    {
        static void Main()
        {
            // UNiDAYS will provide your partnerId and signingKey. This GUID needs to be Base64 encoded before passing it to the DirectTrackingDetailsBuilder
...
A URL will be returned to you which can be placed within a script element on your post-payment/order-success page to call the API. If successful a a response with a status code of 200 OK will be returned. This will only work for `GET` requests.
